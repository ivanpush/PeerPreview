  ...
  return (
    <div className="h-screen flex flex-col bg-[#151515]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="h-44 border-t border-[#2E2E2E] bg-[#1D1D1D]">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {manuscript?.figures?.map((fig, idx) => (
                <button
                  ...
                  className="flex-shrink-0 px-3 py-2 bg-[#252525] hover:bg-[#2A2A2A] text-gray-300 text-sm rounded border border-[#2E2E2E] transition"
                >
                  {fig.label || `Figure ${idx + 1}`}
                </button>
              ))}
              {(!manuscript?.figures || manuscript.figures.length === 0) && (
                <p className="text-sm text-gray-600">No figures available</p>
              )}
            </div>
          </div>
        </div>
